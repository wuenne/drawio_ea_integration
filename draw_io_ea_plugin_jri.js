Draw.loadPlugin(function(ui) {
    ...
    mxResources.parse('importFromEA_JRI=Import XML from Enterprise Architect (JRI Version)...');
    var menu = ui.menus.get('extras');
    if (menu != null) {
        var oldFunct = menu.funct;
        menu.funct = function(menu, parent) {
            oldFunct.apply(this, arguments);
            ui.menus.addMenuItem(menu, ['importFromEA_JRI'], parent);
        };
    }

    ui.actions.addAction('importFromEA_JRI', function() {
    ...
    function createMxGraphCells(umlElements, graph, parent) {
        ...
        for (var i = 0; i < umlElements.length; i++) {
            ...
            if (item.type == 'Class') {
                cell = graph.insertVertex(parent, item.id, item.name + " (NEU)", item.geometry.x, item.geometry.y, item.geometry.width, item.geometry.height);
